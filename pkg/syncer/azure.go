...
func (a *AzureSyncer) Validate() error {
	...
	// When not using client secret auth, verify that the federated token file is available
	...
	if !hasClientSecret {
		...
		if !hasTokenFile {
			validationErrors = append(validationErrors, fmt.Errorf("Could not find `AZURE_CLIENT_SECRET` or `AZURE_FEDERATED_TOKEN_FILE` in secret or as environment variables"))
		}
	}
...
func (a *AzureSyncer) Bind() error {
	...
	clientSecret, hasClientSecret := getSecretOrEnvValue(a.CredentialsSecret, ClientSecret)
